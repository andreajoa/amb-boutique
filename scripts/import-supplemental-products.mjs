...
const products = records.map((record, index) => {
  ...
  return { slug, name: record.name, vendor: "AMB BOUTIQUE", category, price: Number(record.price), ...(record.compare_at ? { compareAt: Number(record.compare_at) } : {}), ...(record.badge ? { badge: record.badge } : {}), sheet: "one", quadrant: 1, colors: colorParts.map((item) => item.value), colorNames: colorParts.map((item) => item.name), sizes: split(record.sizes), description: record.description, materials: record.materials, care: record.care, images: split(record.image_urls), ...(record.gallery_sprite.toLowerCase() === "true" ? { gallerySprite: true } : {}), stock: Number(record.stock || 0), ...(economics[0] !== undefined ? { weightOz: economics[0] } : {}), ...(economics[1] !== undefined ? { unitCostUsd: economics[1] } : {}), ...(economics[2] !== undefined ? { inboundFreightUsd: economics[2] } : {}), ...(economics[3] !== undefined ? { dutyUsd: economics[3] } : {}), ...(economics[4] !== undefined ? { packagingUsd: economics[4] } : {}), ...(economics[5] !== undefined ? { minimumMarginPercent: economics[5] } : {}), ...(record.complementary_slugs ? { complementarySlugs: split(record.complementary_slugs) } : {}), ...(record.stripe_price_id ? { stripePriceId: record.stripe_price_id } : {}) };
});
