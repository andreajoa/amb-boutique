...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!product) return {};
  const description = `${product.name} by AMB BOUTIQUE. Contemporary women’s ${product.category.toLowerCase()} curated in San Diego, California.`;
  return { title: product.name, description, alternates: { canonical: `/products/${product.slug}` }, openGraph: { type: "website", title: product.name, description, url: `/products/${product.slug}`, images: ["/images/product-gallery.webp"] } };
}
