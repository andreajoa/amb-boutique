...
const shoeGalleryViews = [
  { backgroundSize: "contain", backgroundPosition: "center" },
  { backgroundSize: "128%", backgroundPosition: "center" },
  { backgroundSize: "112%", backgroundPosition: "left center" },
  { backgroundSize: "112%", backgroundPosition: "right center" },
];
      ...
      <div className="product-layout shell" data-reveal>
        <section className="product-gallery" aria-label={`${product.name} gallery`}>
          {gallery.map((image, index) => {
            const shoeView = isShoe ? shoeGalleryViews[index % shoeGalleryViews.length] : undefined;
            return <button key={image || index} className={`gallery-image gallery-q${index + 1}`} style={image ? { backgroundImage: `url(${image})`, backgroundSize: shoeView?.backgroundSize || "contain", backgroundPosition: shoeView?.backgroundPosition || "center", backgroundRepeat: "no-repeat", backgroundColor: "#f5f3ef" } : undefined} aria-label={`Open ${product.name} image ${index + 1}`}><span>⌕</span></button>;
          })}
        </section>
        ...
        <section className="product-info">
          ...
          <fieldset className="option-group"><legend><strong>{isShoe ? "EU Size" : "Size"}:</strong> {size}</legend><div className="size-options">{sizes.map((item) => <button type="button" key={item} className={size === item ? "selected" : ""} onClick={() => setSize(item)}>{item}</button>)}</div><div className="size-help-links">{isShoe ? <Link className="shoe-size-link" href="/size-guide#shoes">Shoe Size Guide</Link> : <SizeFinder product={product} sizes={sizes} onSelect={setSize}/>}</div></fieldset>
          ...
          {isShoe ? <div className="shoe-fit-summary"><div><span>Size system</span><strong>EU sizing</strong></div><div><span>Style</span><strong>{product.subcategory || "Shoes"}</strong></div>{product.heelHeightCm ? <div><span>Heel height</span><strong>{product.heelHeightCm} cm</strong></div> : null}<div><span>Fit tip</span><strong>Measure foot length first</strong></div></div> : <div className="fit-guide"><span>How it fits</span><div><i/><i/><i className="active"/><i/><i/></div><p><small>Slim fit</small><small>Regular fit</small><small>Oversized</small></p></div>}
