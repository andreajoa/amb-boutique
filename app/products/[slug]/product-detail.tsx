...
export default function ProductDetail({ product }: { product: Product }) {
  ...
  const [inventoryState, setInventoryState] = useState<{ key: string; managed: boolean; available: number } | null>(null);
  const [inventoryProblem, setInventoryProblem] = useState<{ key: string; message: string } | null>(null);
  ...
  const inventoryKey = `${product.slug}:${color.name}:${size}`;
  const inventory = inventoryState?.key === inventoryKey ? inventoryState : null;
  const inventoryError = inventoryProblem?.key === inventoryKey ? inventoryProblem.message : "";
  ...
  useEffect(() => {
    ...
    fetch(`/api/inventory?${params.toString()}`, { signal: controller.signal, cache: "no-store" })
      .then(async (response) => {
        ...
        setInventoryState({ key: inventoryKey, managed: Boolean(result.managed), available: Math.max(0, Number(result.available) || 0) });
      })
      .catch((error) => {
        if (error instanceof DOMException && error.name === "AbortError") return;
        setInventoryProblem({ key: inventoryKey, message: error instanceof Error ? error.message : "Live availability could not be confirmed." });
      });
    return () => controller.abort();
  }, [product.slug, color.name, size, inventoryKey]);
  ...
  const selectedQuantity = Math.min(quantity, Math.max(1, maximumQuantity));

  const addToBag = () => {
    if (!purchasable) return;
    addItem(product, { size, color: color.name, quantity: selectedQuantity });
  };
    ...
    <main>
      ...
      <div className="product-layout shell" data-reveal>
        ...
        <section className="product-info">
          ...
          <div className="quantity-block"><span>Quantity</span><div><button type="button" onClick={() => setQuantity(Math.max(1, selectedQuantity - 1))} aria-label="Decrease quantity" disabled={!purchasable || selectedQuantity <= 1}>−</button><span>{selectedQuantity}</span><button type="button" onClick={() => setQuantity(Math.min(maximumQuantity, selectedQuantity + 1))} aria-label="Increase quantity" disabled={!purchasable || selectedQuantity >= maximumQuantity}>+</button></div></div>
          ...
          <button className="add-button" type="button" onClick={addToBag} disabled={!purchasable}>Add to Bag · {formatMoney(product.price * selectedQuantity)}</button>
          <button className="buy-button" type="button" disabled={!purchasable} onClick={() => void buyNow(product, { size, color: color.name, quantity: selectedQuantity })}>Buy Now</button>
